...
public class AccountServiceImpl implements AccountService {
    ...
    @Autowired
    private PrimaryMailRepository primaryMailRepository;

    @Autowired
    private MediumMailgunRepository mediumMailgunRepository;
    ...
    @Value("${deskplus.primary-mail-domain}")
    ...
    public Response signup(SignupRequest signupRequest) {
        ...
        if(companyName == null || companyName.isEmpty()){
            return new FailureResponse(ResponseConstants.ERR_COMPANY_NAME_EMPTY, ResponseConstants.ERR_COMPANY_NAME_EMPTY_MSG);
        }
        RegistrationsEntity regEntity = registrationsRepository.getRegistrationByCompayName(companyName);
        if(regEntity != null){
            return new FailureResponse(ResponseConstants.ERR_COMPANY_ALREADY_REGISTERED, ResponseConstants.ERR_COMPANY_ALREADY_REGISTERED_MSG);
        }
        ...
        registrationsEntity.setCompany_name(companyName);
        registrationsEntity.setActivated(false);
        registrationsEntity = registrationsRepository.saveAndFlush(registrationsEntity);

        String primaryMailId = companyName+"-support@"+mailDomain;
        PrimaryMailEntity primaryMailEntity = new PrimaryMailEntity();
        primaryMailEntity.setMail_id(primaryMailId);
        primaryMailEntity.setRegistration(registrationsEntity);
        primaryMailEntity.setActive(true);
        primaryMailEntity = primaryMailRepository.saveAndFlush(primaryMailEntity);

        String mediumMailgunMailId = companyName+"-help-service@"+mailDomain;
        MediumMailgunEntity mediumMailgunEntity = new MediumMailgunEntity();
        mediumMailgunEntity.setMail_id(mediumMailgunMailId);
        mediumMailgunEntity.setRegistration(registrationsEntity);
        mediumMailgunEntity =  mediumMailgunRepository.saveAndFlush(mediumMailgunEntity);
