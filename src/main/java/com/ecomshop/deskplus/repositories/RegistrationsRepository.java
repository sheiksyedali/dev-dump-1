...
public interface RegistrationsRepository extends JpaRepository<RegistrationsEntity, Long> {
    ...
    @Query("SELECT r FROM RegistrationsEntity r WHERE r.company_name = :companyName")
    RegistrationsEntity getRegistrationByCompayName(@Param("companyName") String companyName);
}
